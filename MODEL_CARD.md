...
| **Version** | v0.9.5 + nine differentiation bets, multi-modal-in-security (2026-05-08). v0.9 chat is the ghost-small bench winner (CTIBench full 28.9%, CTF eval 59.2%, SecQA 39.3%, fact recall 1/100 on v2). The 81M ghost-small line saturates as a register-matching parrot. **v0.9.5 ships nine concrete bets to be different**, strategic frame at [`docs/differentiation.md`](docs/differentiation.md). The first six (v0.9.4): tool-grounded SFT (1), daily LoRA over fresh threat-intel (2), custom 32K BPE (3, settled at +4%/-2.5% per [`docs/bpe_corpus_ablation.md`](docs/bpe_corpus_ablation.md)), RoPE NTK 16K context (4), MoE for ghost-1B+ (5, smoke PASS per [`docs/moe_training_smoke.md`](docs/moe_training_smoke.md), presets `ghost-1b` 2.1B/1.2B-active and `ghost-3b` 6.0B/3.3B-active), structured-format pretrain (6, v0.9 baseline 0/32 = [0.0-10.7%] CI per [`docs/format_baseline_v09.md`](docs/format_baseline_v09.md)). The new three (v0.9.5): code-for-security (7, [`docs/code_security_synth.md`](docs/code_security_synth.md)), binary/hex literacy (8, [`docs/binary_literacy_synth.md`](docs/binary_literacy_synth.md), the most novel bet, no other small cybersec LM does this), and provenance with `<\|cite\|>...<\|/cite\|>` tags (9, [`docs/provenance_synth.md`](docs/provenance_synth.md)). Combined templated-synth corpus: 1,505 records, 99.4% parser-pass, all deterministic and reproducible from public banks. The v1.0 corpus has been built (516,736 train / 27,049 val / ~363M tokens / six domains). Ghost-base launcher (`scripts/train_ghost_base.py`, ~360M params) shipped, gated on rented GPU. |
...
| `ghost-small-v0.9` | 6 | 768 | 12 | 3072 | 512 | ~81M | v0.7 arch on 273M-token corpus (PRIMUS + CWE + OWASP + RFCs + fact-QA). Pretrain val 3.638. Chat is the canonical ghost-small bench winner: **28.9% on debiased CTIBench full (n=2500), 59.2% on in-repo CTF MCQ (n=30), 39.3% on SecQA (n=210, external)**. Free-form fact recall at floor on both rulers: **1/50 on v1, 1/100 on v2** with the smarter grader. MCQ wins measure register, not facts. |
| `ghostlm/ghost-base` | 30 | 960 | 15 | 3200 | 1024 | ~360M | Planned (rented GPU). SmolLM2-360M shape; the parameter rung where literature reports factual recall on cybersec MCQ starting to emerge |
