...
| **Version** | v0.9.2 + v1.0 corpus + 2026-05-07 pre-GPU push. v0.9 chat is the ghost-small bench winner (CTIBench full 28.9%, CTF eval 59.2%, SecQA 39.3%, fact recall 1/50 on v1, **1/100 on v2**). The 81M ghost-small line saturates as a register-matching parrot; free-form fact recall is at floor on both v1 and v2 benches. The v1.0 corpus has been built (516,736 train / 27,049 val / ~363M tokens / six domains) and the ghost-base launcher (`scripts/train_ghost_base.py`, 30L × 960d × 15h × 3200 d_ff, SmolLM2-360M shape, ~360M params) is shipped. **Fact-recall v2** at [`Ghostgim/cybersec-fact-recall`](https://huggingface.co/datasets/Ghostgim/cybersec-fact-recall) (n=100 seed, smarter grader: boundary-match + disqualifiers + composite-fact requirements). RAG layer wired into the demo Space (BGE-small embeddings over an 83K-chunk corpus index). Distillation pipeline ready for ghost-3B+ corpus expansion at [`docs/distillation.md`](docs/distillation.md). Multi-year hardware pathway through ghost-7B at [`docs/hardware_pathway.md`](docs/hardware_pathway.md). v1.0 training gated on rented-GPU access. |
