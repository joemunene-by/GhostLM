...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--curriculum-manifest", default=None,
                   help="Path to a train.domains.json (from "
                        "`pretokenize.py --by-domain`) to enable multi-stage "
                        "domain-weighted (curriculum) training.")
    p.add_argument("--curriculum-spec", default=None,
                   help="Compact curriculum spec overriding the default "
                        "generalist schedule, e.g. "
                        "'0.5:general_web=5,cybersec=2;1.0:code=3,math=2'.")
    return p.parse_args()
...
def main() -> None:
    ...
    # Curriculum (multi-stage domain-weighted) training: replace the flat
    # train loader with a domain-weighted sampler over per-domain bins. The
    # mixture shifts with training progress (step / max_steps), the SmolLM2 /
    # H2O-Danube3 recipe. Val stays the flat bin so val loss is comparable.
