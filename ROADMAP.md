...
### Bet 8 binary-literacy expansion shipped (v0.9.22)

`data/raw/binary_literacy_patterns.jsonl` grows from 15 to 40 patterns across 7 categories (was 5): the original file_magic / packer / shellcode / pe_field / disassembly plus new elf_field / encoding / hash. New file magics: JPEG, GIF89a, MP4 ftyp, Java class CAFEBABE, WebAssembly, GZIP, SQLite, Android DEX. New shellcode + asm: x86 execve('/bin/sh') 23-byte stub, ARM64 prologue, x64 epilogue, ROP gadget pop rdi+ret, x64 reverse-shell stub start. New PE/ELF fields: PE Import Address Table, ELF PT_LOAD, ELF PT_INTERP, PE .text characteristics. Encoding: base64, hex string, UTF-8 BOM. Hash recognition: MD5/SHA-256 length, bcrypt format. Held-out eval grows from 20 to 35 prompts. SFT corpus produced by `synth_binary_literacy.py` reaches **109 records** (was 44). 11 new test cases bringing the suite to 260, all green. The previously-thin bet 8 is now comparable to bet 12 protocol (60 patterns / 240 SFT records target).

### Code-explain templated synth shipped (v0.9.23)

`scripts/synth_code_explain.py` introduces a fifth templated-synth bet (alongside bet 6, 7, 8, 12) for code explanation. 40 hand-curated patterns covering algorithms (recursive/memoized/iterative Fibonacci, binary search, BFS/DFS, merge sort, top-K), data structures (linked list, BST, queue-from-stacks, LRU cache, Counter, defaultdict), idioms (Python contextmanager, Go defer, Rust `?`, JS optional chaining, Rust match, list comprehension), patterns (singleton, factory, observer, builder, strategy, decorator factory, trait-object polymorphism), and concurrency / I/O (asyncio.gather, atomic file write, Go HTTP server, regex, iterator chain, set algebra) across Python, JS, TS, Go, Rust, Java. 5 variants per pattern (pretrain_prose, identify_lang, explain_purpose, walkthrough, concepts) produce **200 records**. 15 held-out eval prompts. 8 new test cases bringing the suite to 268, all green. Code SFT total now 618 records, on track to surpass cybersec (~1,940) over the next 6-7 templated-synth releases.

### Code-write templated synth shipped (v0.9.24)
...
### Code-write phase 2 shipped (v0.9.26)

`data/raw/code_write_patterns.jsonl` doubled from 40 to 80 patterns via `scripts/expand_code_write_bank.py`, adding database ops (SQLite param inserts, SQLAlchemy update), testing (pytest, Go table-driven, mock.patch), concurrency (asyncio.Semaphore, Go WaitGroup), file system (rglob, shutil.move, Go filepath.WalkDir), string manipulation (zfill, regex sub, CSV split), math + numbers (statistics, Decimal banker's rounding), date/time (ISO parse, day diff, duration format), hashing + encoding (SHA-256 chunked, URL-safe base64, HMAC compare_digest), HTTP (JSON POST, URL build), patterns (context manager class, dataclass field), algorithms (LRU cache, quicksort, BFS shortest path, cycle detection), and multi-language coverage (Rust HashMap, Rust file write, Go worker pool, JS event emitter, Python timeout / chunked streaming / TTL cache / JSON schema / gzip / env vars / UUID4). The existing `synth_code_write.py` produces **243 records** (was 123). Code SFT total now **1,061 records**, ~55% of cybersec target. Roughly halfway to surpass.

### Both code banks phase 3 shipped (v0.9.27)
